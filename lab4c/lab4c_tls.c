...
// a helper function that reports an error message usingreturn value of the ssh function and exit 2
void report_ssl_error_and_exit(const char* msg)
{
	int err_code = SSL_get_error(mssl, mssl_ret); // obtain the actual error code
	fprintf(stderr, "%s: the SSL error code is %d\n", msg, err_code);
	exit(2);
}
...
void off_interrupt_handler()
{
	...
	if (mssl_ret <= 0)
		report_ssl_error_and_exit("Failed to write report to server");
	if (opt_log)
	{
		if (write(log_fd, report, nwrite) < 0)
			report_error_and_exit("Failed to write report to log file");
	}
	// report the final sample
	exit(0);
}

// initialize and free the SSL objects
void SSL_init()
{
	SSL_library_init();
...
int main(int argc, char* argv[])
{
	...
	// intitialize ssl and link the socket with ssl object
	SSL_init();
	...
	// close the socket, mraa, and ssl objects before exiting
	atexit(deallocate);

	char identity[20];
	sprintf(identity, "ID=%d\n", id_number);
	mssl_ret = SSL_write(mssl, identity, strlen(identity));
	if (mssl_ret <= 0)
		report_ssl_error_and_exit("Error writing id to server");
	if (opt_log)
	{
		if (write(log_fd, identity, strlen(identity)) < 0)
			report_error_and_exit("Error writing id to the log file");
	}
	...
	input_fds[0].fd = client_socket_fd;
	...
	while (run_flag)
	{
		...
		else if (poll_ret > 0)
		{
				...
				// find pending commands from stdin 
			{
				...
				mssl_ret = nread;
				if (mssl_ret <= 0)
					report_ssl_error_and_exit("Error reading from server");
				for (i = 0; i < nread; ++i)
				{
					char ch = buffer[i];
					if (ch == '\n')
